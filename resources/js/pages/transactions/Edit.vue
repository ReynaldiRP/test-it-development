...
import { CustomerDetails, ProductDetails, Transaction, TransactionItem } from '@/types/transaction';
...
// Form data - Initialize with existing transaction data
const form = useForm({
    customer_id: props.transaction.customer_id,
    invoice_date: new Date(props.transaction.invoice_date),
    total: props.transaction.total,
    items:
        props.transaction.detail_transactions?.map((item) => ({
...
// Initialize customer details on load
const initializeCustomerDetails = async () => {
    if (form.customer_id) {
        try {
            const response = await axios.get(`/api/customers/${form.customer_id}`);
...
// Get available stock for a product (considering items already in transaction)
const getAvailableStock = (productId: number): number => {
    const product = props.products.find((p) => p.id === productId);
    if (!product) return 0;

    // Add back the quantity if this product is already in the transaction
...
const addItem = () => {
    ...
    const availableStock = getAvailableStock(currentItem.value.product_id);

    if (currentItem.value.quantity > availableStock) {
        toast.add({
            severity: 'error',
            summary: 'Error',
            detail: `Insufficient stock. Available: ${availableStock}`,
            life: 3000,
        });
        return;
    }
    ...
    form.put(route('transactions.update', { transaction: props.transaction.id }), {
        onError: () => {
            toast.add({
                severity: 'error',
                summary: 'Error',
                detail: session.value.error || 'Failed to update transaction',
                life: 3000,
            });
        },
    });
        ...
        <div class="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-2 sm:p-4">
            <div class="relative min-h-[100vh] flex-1 rounded-xl border border-sidebar-border/70 p-3 sm:p-5 md:min-h-min dark:border-sidebar-border">
                ...
                <form @submit.prevent="submit" class="grid gap-6 rounded-md p-4">
                    <div class="mb-6">
                        <h2 class="text-xl font-semibold text-foreground">Edit Transaction</h2>
                        ...
                        <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                            ...
                            <div class="grid gap-2">
                                ...
                                <InputNumber
                                    ...
                                    :min="1"
                                    :max="getAvailableStock(currentItem.product_id)"
                                    :useGrouping="false"
                                    placeholder="Quantity"
                                />
                                <small v-if="selectedProduct" class="text-muted-foreground">
                                    Available stock: {{ getAvailableStock(currentItem.product_id) }}
                                </small>
                            </div>
